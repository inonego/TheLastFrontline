...
namespace MimicSpace
{
    ...
    public class Movement : MonoBehaviour
    {
        ...
        void Update()
        {
            if (target != null)
            {
                Vector3 directionToTarget = (target.position - transform.position).normalized; // 타겟 방향
                velocity = Vector3.Lerp(velocity, directionToTarget * speed, velocityLerpCoef * Time.deltaTime);
            }
            //velocity = Vector3.Lerp(velocity, new Vector3(0, 0, Time.deltaTime).normalized * speed, velocityLerpCoef * Time.deltaTime);
